...
The connector's chassis already speaks FreeAgent OAuth — adding *Magento*-side OAuth on top of that meant:

- Two OAuth flows on every connect (FreeAgent + Magento)
...
**Stores → Configuration → Byte8 → FreeAgent Accounting**
...
2. Stores `SHA-256(code)` in `byte8/freeagent_accounting/pairing_code_hash` and `time()` in `byte8/freeagent_accounting/pairing_code_issued_at`. Plaintext code is **never stored** — it's revealed once via session flash and shown on the next page render.
...
Click **Open Byte8 Ledger** (button beside the pairing code). This deep-links to `ledger.byte8.io/dashboard/connect-magento?provider=freeagent` with a return-to context. The `?provider=freeagent` hint tells the dashboard to set up a FreeAgent binding (vs Sage) — important if you also run Sage Accounting on the same tenant.
...
```
POST /v1/magento/connect?provider=freeagent
...
2. Calls back into Magento at `POST <magento_base_url>/V1/byte8/freeagent/setup/pair`:
   ```
...
4. Returns `200 OK`. The chassis persists its binding row (with `provider = 'freeagent'`) and returns success to the dashboard.
...
- **Single-use** — once the chassis successfully pairs, the stored hash is cleared. A second `POST /V1/byte8/freeagent/setup/pair` with the same code 422s. Prevents a leaked code from rebinding to a different chassis tenant.
...
If you already run `byte8/magento-sage-accounting` paired against the same Byte8 tenant, FreeAgent pairs as a **second binding** on the same tenant — the chassis just adds a `provider = 'freeagent'` binding row alongside the existing `sage_accounting` one. The Magento side stores its FreeAgent `api_key` under `byte8/freeagent_accounting/api_key` (separate config path from the Sage one), so the two providers don't trample each other.
...
- [FreeAgent OAuth](/docs/connect/freeagent-oauth) — what happens after pairing, when you connect your FreeAgent account to the chassis.
