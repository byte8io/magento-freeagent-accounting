...
When the outbox classifies a delivery failure as **deterministic** (a 4xx from the chassis, or 10 consecutive transient retries with no success), the row goes to `dead_lettered` status and a **banner appears on the FreeAgent Accounting config page**:

> ⚠ **3 dead-lettered events** — these failed permanently and won't retry. Use `bin/magento byte8:client:outbox:inspect` to triage.

The banner sits inside the **Connection Status** block at the top of **Stores → Configuration → Byte8 → FreeAgent Accounting**.
...
Shows entity_id, event_name, idempotency_key, last_status_code, last_attempt_at, the `provider` column (so you can see whether each row is Sage or FreeAgent), and the first 200 chars of `last_error`. Most operators run this from a support session.

Filter to FreeAgent only:
...
- **Validation 4xx** — the binding's sync policy referenced a stale FreeAgent income category / bank account URL. Refresh reference cache + correct the policy on the dashboard.
...
- **Provider 422** — FreeAgent rejected the payload for a reason in the catalogue (see [Troubleshooting](/docs/troubleshooting)). Fix the cause, re-queue.
...
2. Manually delete the dead-lettered row (`DELETE FROM byte8_event_outbox WHERE status='dead_lettered' AND provider='freeagent' AND entity_id=…`).
...
```bash
bin/magento byte8:client:outbox:inspect --provider=freeagent | head -50   # see top-50 oldest
