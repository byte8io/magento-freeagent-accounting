...
In the Magento admin: **Stores → Configuration → Byte8 → FreeAgent Accounting**.
...
Click **Open Byte8 Ledger** beside the pairing-code field — it redirects you straight to `ledger.byte8.io/dashboard/connect-magento?provider=freeagent` with the right tenant context.
...
Submit. Ledger calls back into your Magento at `POST /V1/byte8/freeagent/setup/pair`, persists the per-tenant `api_key`, and the Magento Connection Status block flips to green ("Connected").

## 4. Connect FreeAgent

Still on `ledger.byte8.io`, navigate to **Connect → FreeAgent** and complete the standard FreeAgent OAuth handshake. The chassis stores the OAuth tokens encrypted at rest and refreshes them automatically — you never see them again.

After connecting, the chassis builds a **reference cache** for your FreeAgent account: income categories, bank accounts, payment-method targets. This takes ~3-5 seconds and runs once per binding; the dashboard's settings dropdowns populate from it.
...
- **In your Magento admin** — Sales → Invoices grid, with the **FreeAgent Status** chip flipping from `⏳ Pending` to `✓ Synced`.
- **In FreeAgent** — listed as an Open invoice on the contact created from the Magento customer.
- **On the Byte8 dashboard** — `ledger.byte8.io/dashboard/sync` shows a `succeeded` row with the FreeAgent invoice URL (`https://api.freeagent.com/v2/invoices/<uuid>`).
...
- [Browse the Magento admin surfaces](/docs/magento-admin/freeagent-status-grid) — chips, info blocks, dead-letter banner, every visual confirmation you'll see day-to-day.
