...
| Outbound HTTPS | port 443 | To `ledger.byte8.io` only — no direct connection to FreeAgent |
...
- A FreeAgent account that you control (you OAuth your own FreeAgent company into the chassis after pairing the Magento side).

The connector itself doesn't store any credentials in PHP — FreeAgent OAuth tokens live encrypted in our hosted ledger. The only secret stored on your Magento side is the per-tenant `api_key` (encrypted via Magento's `EncryptorInterface` in `core_config_data`), and even that is set automatically by the pairing handshake.
...
`composer require byte8/magento-freeagent-accounting` transitively pulls:
...
Three Magento modules end up enabled: `Byte8_Core`, `Byte8_Client`, `Byte8_FreeAgentAccounting`. Verify with:
...
If you already run `byte8/magento-sage-accounting` on this install, **both connectors coexist on the same Magento** — they share the `byte8/module-client` chassis. The outbox grows a `provider` column (default `sage_accounting`) and routes events per provider; grid join aliases are suffixed (`byte8_sync_status_freeagent` vs the Sage column) so the two FreeAgent + Sage Status chips can sit side-by-side on Sales → Invoices.
...
```bash
bin/magento module:status Byte8_FreeAgentAccounting
# → Module is enabled

bin/magento config:show byte8/freeagent_accounting/tenant_id
# → empty (you haven't paired yet — fine)

bin/magento dev:tests:run unit -- Byte8/FreeAgentAccounting   # optional sanity check
```

In the admin: **Stores → Configuration → Byte8 → FreeAgent Accounting** should render a config page with a red "Not connected" status block + a **Generate pairing code** button. That's the install verification.

## Database changes

`bin/magento setup:upgrade` creates two tables (both owned by `byte8/module-client`):

- `byte8_event_outbox` — transport queue for outbound events. Drained by the `byte8_outbox_drain` cron every minute. Carries a `provider` column so Sage + FreeAgent events route correctly.
- `byte8_entity_sync_state` — Magento-side mirror of ledger sync outcomes. Drives the FreeAgent Status chips on the admin grids and detail pages.

The connector itself adds Magento config rows under `byte8/freeagent_accounting/*` for the pairing flow but no new tables.
...
This stops the FreeAgent observers from firing and removes the module from the autoload. The `byte8_event_outbox` and `byte8_entity_sync_state` tables stay (audit data) and the shared `Byte8_Client` / `Byte8_Core` chassis stays enabled if Sage is still installed; drop them manually if you want a clean slate. Disconnect on the ledger side from **Stores → Configuration → Byte8 → FreeAgent Accounting → Disconnect** *before* removing the module so the chassis flips its binding row to `revoked`.

## Network requirements

- **Outbound from Magento:** HTTPS to `ledger.byte8.io` (port 443) only. No direct connection to `api.freeagent.com` from Magento.
